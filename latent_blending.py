...
class LatentBlending:
    def __init__(
        ...
        self.sdh: StableDiffusionHolder = sdh
    ...
    def set_image_to_image(self, img1, img2):
        ...
        self.ccA = self.sdh.model.get_first_stage_encoding(
            self.sdh.model.encode_first_stage(self.img1)
        )
        self.ccB = self.sdh.model.get_first_stage_encoding(
            self.sdh.model.encode_first_stage(self.img2)
        )
    ...
    def load_branching_profile(
        ...
        self.autosetup_branching(
            depth_strength, num_inference_steps, nmb_branches_final
        )
    ...
    def autosetup_branching(
        ...
        list_injection_idx.extend(
            np.linspace(idx_injection_first, idx_injection_last, nmb_injections).astype(
                int
            )
        )
        list_nmb_branches = np.round(
            np.logspace(np.log10(2), np.log10(nmb_branches_final), nmb_injections + 1)
        ).astype(int)
        ...
        for idx_injection, nmb_branches in zip(
            list_injection_idx[1:], list_nmb_branches[1:]
        ):
            if (
                idx_injection - list_injection_idx_clean[-1] >= nmb_mindist
                and nmb_branches > list_nmb_branches_clean[-1]
            ):
        ...
        print(
            f"autosetup_branching: num_inference_steps: {num_inference_steps} list_nmb_branches: {list_nmb_branches} list_injection_idx: {list_injection_idx}"
        )
        self.setup_branching(
            num_inference_steps,
            list_nmb_branches=list_nmb_branches,
            list_injection_idx=list_injection_idx,
        )
    ...
    def setup_branching(
        ...
        # Assert
        assert not (
            (list_injection_strength is not None) and (list_injection_idx is not None)
        ), "suppyl either list_injection_strength or list_injection_idx"

        if list_injection_strength is None:
            assert (
                list_injection_idx is not None
            ), "Supply either list_injection_idx or list_injection_strength"
            assert isinstance(list_injection_idx[0], int) or isinstance(
                list_injection_idx[0], np.int
            ), "Need to supply integers for list_injection_idx"

        if list_injection_idx is None:
            assert (
                list_injection_strength is not None
            ), "Supply either list_injection_idx or list_injection_strength"
            # Create the injection indexes
            list_injection_idx = [
                int(round(x * num_inference_steps)) for x in list_injection_strength
            ]
            assert (
                min(np.diff(list_injection_idx)) > 0
            ), "Injection idx needs to be increasing"
            if min(np.diff(list_injection_idx)) < 2:
                print(
                    "Warning: your injection spacing is very tight. consider increasing the distances"
                )
            assert isinstance(list_injection_strength[1], np.floating) or isinstance(
                list_injection_strength[1], float
            ), "Need to supply floats for list_injection_strength"
            # we are checking element 1 in list_injection_strength because "0" is an int... [0, 0.5]

        assert (
            max(list_injection_idx) < num_inference_steps
        ), "Decrease the injection index or strength"
        assert len(list_injection_idx) == len(
            list_nmb_branches
        ), "Need to have same length"
        assert (
            max(list_injection_idx) < num_inference_steps
        ), "Injection index cannot happen after last diffusion step! Decrease list_injection_idx or list_injection_strength[-1]"
        ...
        assert (
            list_nmb_branches[0] == 2
        ), "Need to start with 2 branches. set list_nmb_branches[0]=2"
    ...
    def init_tree_struct(self):
        ...
        for t_block in range(nmb_blocks_time):
            ...
            list_fract_mixing_current = get_spacing(
                nmb_branches, self.mid_compression_scaler
            )
    ...
    def run_transition(
        ...
        assert (
            self.list_injection_idx is not None
        ), "Set the branching structure before, by calling autosetup_branching or setup_branching"
        ...
        if self.branch1_influence > 0.0 and not self.branch1_insertion_completed:
            assert (
                self.list_nmb_branches[0] == 2
            ), "branch1 influnce currently requires the self.list_nmb_branches[0] = 0"
            ...
            idx_crossfeed = int(
                round(self.list_injection_idx[1] * self.branch1_fract_crossfeed)
            )
        ...
        if not recycle_img1 and not recycle_img2:
            ...
            for t_block in range(nmb_blocks_time):
                ...
                if recycle_img1:
                    ...
                    self.tree_final_imgs[0] = self.sdh.latent2image(
                        self.tree_latents[-1][0][-1]
                    )
                    self.tree_final_imgs_timing[0] = 0
                if recycle_img2:
                    self.tree_status[t_block][-1] = "computed"
                    self.tree_final_imgs[-1] = self.sdh.latent2image(
                        self.tree_latents[-1][-1][-1]
                    )
        ...
        for idx_leaf in range(1, self.list_nmb_branches[-1]):
            ...
            for t_block in range(nmb_blocks_time - 1, 0, -1):
                ...
                b_parent1, b_parent2 = get_closest_idx(
                    fract_mixing, list_fract_mixing_prev
                )
                assert (
                    self.tree_status[t_block_prev][b_parent1] != "untouched"
                ), "Branch destruction??? This should never happen!"
        ...
        for t_block, idx_branch in tqdm(
            list_compute, desc="computing transition", smoothing=0.01
        ):
            ...
            if idx_branch > premature_stop:
                print(
                    f"run_transition: premature_stop criterion reached. returning tree with {premature_stop} branches"
                )
                return self.tree_final_imgs
            ...
            if t_block == 0:
                ...
                if idx_branch == 1 and self.branch1_influence > 0:
                    ...
                    for i in range(len(list_latents)):
                        list_latents[i] = interpolate_spherical(
                            list_latents[i],
                            self.tree_latents[0][0][i],
                            fract_base_influence,
                        )
            else:
                # find parents latents
                b_parent1, b_parent2 = get_closest_idx(
                    fract_mixing, self.tree_fracts[t_block - 1]
                )
                ...
                fract_mixing_parental = (
                    fract_mixing - self.tree_fracts[t_block - 1][b_parent1]
                ) / (
                    self.tree_fracts[t_block - 1][b_parent2]
                    - self.tree_fracts[t_block - 1][b_parent1]
                )
                latents_for_injection = interpolate_spherical(
                    latents1, latents2, fract_mixing_parental
                )
                list_latents = self.run_diffusion(
                    list_conditionings,
                    latents_for_injection,
                    idx_start=idx_start,
                    idx_stop=idx_stop,
                )
            ...
            if t_block == nmb_blocks_time - 1:
                self.tree_final_imgs[idx_branch] = self.sdh.latent2image(
                    list_latents[-1]
                )
    ...
    def run_multi_transition(
        ...
        assert len(list_prompts) == len(
            list_seeds
        ), "Supply the same number of prompts and seeds"
        ...
        for i in range(len(list_prompts) - 1):
            ...
            list_imgs = self.run_transition(
                recycle_img1=recycle_img1, fixed_seeds=local_seeds
            )
            list_imgs_interp = add_frames_linear_interp(
                list_imgs, fps, duration_single_trans
            )
    ...
    def run_diffusion(
        ...
        if self.image_to_image:
            ...
            return self.sdh.run_diffusion_img2img(
                text_embeddings,
        ...
        if self.mode == "standard":
            ...
            text_embeddings = list_conditionings[0]
            assert (
                self.sdh.image_source is not None
            ), "image_source is None. Please run init_inpainting first."
            assert (
                self.sdh.mask_image is not None
            ), "image_source is None. Please run init_inpainting first."
            return self.sdh.run_diffusion_inpaint(
                text_embeddings,
                latents_for_injection=latents_for_injection,
                idx_start=idx_start,
                idx_stop=idx_stop,
    ...
    def run_upscaling_step2(
        ...
        assert os.path.isfile(
            fp_yml
        ), "lowres.yaml does not exist. did you forget run_upscaling_step1?"
        ...
        for i in range(nmb_images_lowres):
            ...
            assert os.path.isfile(
                fp_img_lowres
            ), f"{fp_img_lowres} does not exist. did you forget run_upscaling_step1?"
        ...
        for i in range(nmb_images_lowres - 1):
            ...
            self.text_embedding1 = interpolate_linear(
                text_embeddingA, text_embeddingB, list_fract_mixing[i]
            )
            self.text_embedding2 = interpolate_linear(
                text_embeddingA, text_embeddingB, 1 - list_fract_mixing[i]
            )
    ...
    def get_mixed_conditioning(self, fract_mixing):
        ...
        if self.image_to_image:
            text_embeddings_mix = interpolate_linear(
                self.text_embedding1, self.text_embedding2, fract_mixing
            )
            print("get cond1")
            print("get cond2")
            cond["c_concat"][0] = interpolate_spherical(
                self.ccA["c_concat"][0], self.ccB["c_concat"][0], fract_mixing
            )
            ...
            cond["c_concat"][0] = interpolate_spherical(
                cond["c_concat"][0], condB["c_concat"][0], fract_mixing
            )
            uc_full["c_concat"][0] = interpolate_spherical(
                uc_full["c_concat"][0], uc_fullB["c_concat"][0], fract_mixing
            )
    ...
    def inject_latents(self, list_latents, inject_img1=True, inject_img2=False):
        ...
        assert (
            self.tree_latents is not None
        ), "You need to setup the branching beforehand, run autosetup_branching() or setup_branching() before"

        for t_block in range(len(self.list_injection_idx)):
            if inject_img1:
                self.tree_latents[t_block][0] = list_latents[
                    self.list_injection_idx_ext[t_block] : self.list_injection_idx_ext[
                        t_block + 1
                    ]
                ]
            if inject_img2:
                self.tree_latents[t_block][-1] = list_latents[
                    self.list_injection_idx_ext[t_block] : self.list_injection_idx_ext[
                        t_block + 1
                    ]
                ]
