...
def make_batch_inpaint(image, mask, txt, device, num_samples=1):
...
class StableDiffusionHolder:
    def __init__(
        ...
        # Inpainting inits
        self.mask_empty = Image.fromarray(
            255 * np.ones([self.width, self.height], dtype=np.uint8)
        )
        self.image_empty = Image.fromarray(
    ...
    def init_model(self, fp_ckpt, fp_config):
        ...
        assert os.path.isfile(
            fp_ckpt
        ), f"Your model checkpoint file does not exist: {fp_ckpt}"
        ...
        if fp_config is None:
            ...
            if "depth" in fn_ckpt:
                ...
                raise ValueError(
                    "auto detect of config failed. please specify fp_config manually!"
                )

            assert os.path.isfile(
                fp_config
            ), "Auto-init of the config file failed. Please specify manually."

        assert os.path.isfile(
            fp_config
        ), f"Your config file does not exist: {fp_config}"
    ...
    def init_inpainting(
        ...
        if not init_empty:
            ...
            ), "init_inpainting: you need to provide image_source"
            assert (
                mask_image is not None
            ), "init_inpainting: you need to provide mask_image"
    ...
    @torch.no_grad()
    def get_cond_upscaling(
        self, image, text_embedding, noise_level, image_to_image=False
    ):
        r"""
        ...
        w, h = image.size
        noise_level = (
            torch.Tensor(1 * [noise_level]).to(self.sampler.model.device).long()
        )
        batch = make_batch_superres(
            image, txt="placeholder", device=self.device, num_samples=1
        )
    ...
    def run_diffusion_img2img(
        ...
        self.sampler.make_schedule(
            ddim_num_steps=self.num_inference_steps - 1,
        ...
        with precision_scope("cuda"):
            with self.model.ema_scope():
                ...
                z_enc = self.sampler.stochastic_encode(
                    init_latent, torch.tensor([t_enc] * 1).to(self.device)
                )
                # decode it
        ...
        with precision_scope("cuda"):
            with self.model.ema_scope():
                ...
                for i, step in enumerate(time_range):
                    ...
                    outs = self.sampler.p_sample_ddim(
                        latents,
                        text_embeddings,
                        ts,
                        index=index,
    ...
    @torch.no_grad()
    def latent2image(self, latents: torch.FloatTensor):
        r"""
